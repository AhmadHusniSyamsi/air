...
class GroundCheck(db.Model):
    ...
    lokasi = db.Column(db.String(50), nullable=False)
    tanggal = db.Column(db.Date, nullable=False)
    teknisi = db.Column(db.Text)  # bisa kosong
    paraf = db.Column(db.Text)    # bisa kosong
    catatan = db.Column(db.Text)  # bisa kosong
    rows = db.relationship(
        "GroundCheckRow", backref="ground_check", cascade="all, delete-orphan"
    )

class GroundCheckRow(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    groundcheck_id = db.Column(db.Integer, db.ForeignKey("ground_check.id"))
    freq = db.Column(db.String(10))   # 90 Hz / 150 Hz / Center
    jarak = db.Column(db.Float, nullable=True)   # boleh kosong
    degree = db.Column(db.String(10), nullable=True)  # ubah dari Float ke String
